...
public class SemanticAnalyzer {
    ...
    // crea una estructura de datos para el bloque de tipos
    ...
    public static void checkVariable(Gui gui1, String type, String id, int line) {
        ...
        // A.  busca el id en la tabla de simbolos
        if (!symbolTable.containsKey(id)) {
            // B. si  ! existe entonces inserta: type, scope=global, value={0, false, "", '')
            Vector v = new Vector();
            v.add(new SymbolTableItem(type, "global", ""));
            symbolTable.put(id, v);
        } // C. else error: la variable id se encuentra definda
        else {
    ...
    public static void error(Gui gui, int err, int n, String info) {
        switch (err) {
            case 1:
                gui.writeConsole("Linea" + n + ": variable: " + info + "> Ya se encuentra definida");
                break;
            case 2:
                gui.writeConsole("Linea" + n + ": tipos incompatibles: falta de coincidencia de tipos");
                break;
            case 3:
                gui.writeConsole("Linea" + n + ": tipos incompatibles: se esperaba boolean");
                break;
            case 4:
                gui.writeConsole("Linea" + n + ": variable <" + info + "> no encontrada ");
                break;
